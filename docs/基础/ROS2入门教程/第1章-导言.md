...
![F1.1](../../../static/img/ROS2入门教程/1/F1.png "图 1.1：机器人软件层的表示。")
...
![F1.2](../../../static/img/ROS2入门教程/1/F2.png "图 1.2：截至 2021 年 8 月交付的 ROS2 分配。")
...
![F1.3](../../../static/img/ROS2入门教程/1/F3.png "图 1.3：计算机图形图表中使用的符号说明。")

第一个计算图如图 1.4 所示，是一个与 Kobuki[↻](http://kobuki.yujinrobot.com/about2) 机器人交互的简单程序示例。

![F1.4](../../../static/img/ROS2入门教程/1/F4.png "图 1.4： Kobuki 机器人简单控制的计算图。控制应用程序发布根据其订阅的bumper信息计算出的速度。")
...
>**深入研究： ROS2 中的命名规则**
ROS2 中的资源名称与 Unix 中的文件系统非常相似。在创建资源（例如发布者）时，我们可以指定其相对名称、绝对名称（以"/"开头）或私人名称（以"∼"开头）。此外，我们还可以定义一个命名空间，其目的是将资源与其他命名空间隔离开来，方法是将工作空间的名称添加为名称的第一个组成部分。例如，命名空间在多机器人应用程序中非常有用。下面我们来看一个例子，说明根据节点名称和命名空间得出的话题名称：

|name|Result: (node: my node / ns: none)|Result: (node: my node / ns: my ns)|
|---|---|---|
...
![F1.5](../../../static/img/ROS2入门教程/1/F5.png "图 1.5：使用从机器人 RGBD 摄像机获取的激光数据和预处理信息（人和物体）的控制应用程序计算图。")
...
![F1.6](../../../static/img/ROS2入门教程/1/F6.png "图 1.6：使用导航子系统的蒂亚戈机器人基于行为的应用计算图。")
...
## 1.1.3 工作空间
 ...
 &emsp;&emsp;在本书参考的 Ubuntu Linux 22.04 上，可以使用 apt 等工具安装 deb 软件包。每个 ROS2 软件包都打包在一个 deb 软件包中。发行版中 deb 软件包的名称以 `ros-<distro>-<ros2 软件包名称>` 开头，因此很容易识别。为了访问这些软件包，请配置 APT ROS2 软件仓库：  
...
cd bookros2_ws/src
git clone-b foxy-devel https://github.com/fmrico/book_ros2.git #TODO: 修改仓库
```
...
## 1.2 ROS2 的设计
 ...
 ![F1.7](../../../static/img/ROS2入门教程/1/F7.png)
